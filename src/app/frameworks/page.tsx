      ...
      >
        ...
        <div className="flex gap-2 mt-3 flex-wrap">
          {supportedPlatforms.map((platform) => (
            <div
              key={platform}
              className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded text-xs text-gray-300"
            >
              <SupportIndicator support={framework.platforms[platform]} />
              <span>{platform}</span>
            </div>
          ))}
        </div>
...
// DESKTOP TABLE VIEW (keep your existing CoverageMatrix but add a key prop fix)
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-4 justify-center">
            ...
            <motion.a
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="px-6 md:px-8 py-3 md:py-4 border-2 border-white/20 rounded-lg font-semibold text-base md:text-lg text-white hover:bg-white/5 transition-colors"
            >
              Talk to an Expert
            </motion.a>
          </div>
